...
La passe « Rythme et variance » (voir `docs/design/passe-rythme-et-variance-v0-1.md`) clôt les neuf recommandations de l'audit : tour d'observation (un plan vide est légal — l'IA se tait, le monde continue, le soupçon retombe de 4, les crises courent) et variance contrôlée par seed de partie, sous contrat strict : la variance ne porte que sur des choix narratifs entre options éligibles (quel événement, quelle voix), jamais sur des amplitudes — même seed et mêmes choix donnent une partie identique, et l'enveloppe d'équilibrage est vérifiée stable sur quatre seeds.
...
- `docs/design/passe-rapport-lisible-v0-1.md` — rapport d'évolution en deux étages : ~54 mots visibles par tour (était ~280), détail en tiroirs.
- `docs/design/passe-cause-et-effet-v0-1.md` — lisibilité stratégique : pastilles de direction sur les cartes, attribution « Vous / Le monde », filtre « Agir sur », deltas du bandeau. Doctrine : le flou porte sur le second ordre, jamais sur le premier.
